...
def valid(args, model, valset, criterion, writer):
    ...
    with torch.no_grad() :
        for i, (src, label, y_c) in enumerate(testloader):
            ...
            uncertaintyarry_type = np.zeros((1, src.shape[0], 12))
            ...
            for b in range(int(1)):
                ...
                uncertaintyarry_type[b,:,:] = y_hat.cpu()
    ...
    if (np.mean(corr)) > args.corr : 
        ...
        np.savetxt(f'{Folder}/eval_{args.current_epoch}/eval_{args.current_epoch}_acc_{mse:.4f}_month.csv', corr)
