...
class Model_3D(nn.Module):
    def __init__(self, in_channels, out_channels, num_layer, num_answer, drop, input):
        ...
        for l in range(self.layers):
            self.attentions.append(Attention(encoder_dim, decoder_dim, attention_dim))  # attention network
            decode_step = nn.LSTMCell(encoder_dim, decoder_dim, bias=True)  # decoding LSTMCell
            ...
            init_h = nn.Linear(encoder_dim, decoder_dim)  # linear layer to find initial hidden state of LSTMCell
            nn.init.orthogonal_(init_h.weight, gain=gain)
            init_h.bias.data.fill_(0)
            self.init_hs.append(init_h)

            init_c = nn.Linear(encoder_dim, decoder_dim)  # linear layer to find initial hidden state of LSTMCell
            ...
            nn.init.orthogonal_(f_beta.weight, gain=gain)
            f_beta.bias.data.fill_(0)
            self.f_betas.append(f_beta)

            fc = nn.Linear(decoder_dim, self.decode_lengths)  # linear layer to find scores over vocabulary
        ...
        flattenSize =  2592#5184#10368#5184#2592 #3456
    ...
    def forward(self, x) :
        ...
        cs = []
        for l in range(self.layers):
            input_h = self.init_hs[l](mean_encoder_out[0:1, :])  # (batch_size, decoder_dim)
            hs.append(input_h)
            input_c = self.init_cs[l](mean_encoder_out[0:1, :])  # (batch_size, decoder_dim)
            cs.append(input_c)

        overlap = 1
        predictions = torch.zeros(batch_size, (self.decode_lengths-overlap)*self.layers + overlap, 1).to(device)
        alphas = torch.zeros(batch_size, num_pixels, 1).to(device)
        counts = torch.zeros(batch_size, (self.decode_lengths-overlap)*self.layers + overlap, 1).to(device)

        for b in range(batch_size):
            for l in range(self.layers):
                h = hs[l]
                c = cs[l]
                attention_weighted_encoding, alpha = self.attentions[l](encoder_out[b:b+1, :, :], h)
                gate = self.sigmoid(self.f_betas[l](h))
                attention_weighted_encoding = gate * attention_weighted_encoding
                h, c = self.decode_steps[l](attention_weighted_encoding, (h, c))     
                h = MCDropout(h, self.droprate, apply=True)
                preds = self.fcs[l](h)
                idx = (self.decode_lengths-overlap)*l
                predictions[b:b+1, idx:idx + self.decode_lengths, 0] += preds
                alphas[b:b+1, :, 0] = alpha
                counts[b:b+1, idx:idx + self.decode_lengths, 0] += torch.ones_like(preds)
                hs[l] = h
                cs[l] = c
        ...
        out_c = MCDropout(out_c, self.droprate, apply=True)
        out_c = self.linear2(out_c)

        return (predictions, out_c, alphas, firstConv, secondConv, thirdConv)
