...
class stateful(Module):
    ...
    def forward(self, src: Tensor, tgt: Tensor, src_mask: Optional[Tensor] = None, tgt_mask: Optional[Tensor] = None,
        ...
        for i in range(b):
            memory = self.encoder(src[i, :], mask=src_mask, src_key_padding_mask=src_key_padding_mask)
...
class customresnet(nn.Module):
    def __init__(self) -> None:
        ...
        self.ml = resnet.resnet152()
        self.ml.conv1 = nn.Conv2d(2, 64, kernel_size=(7,7), stride=(2,2), padding=(3,3), bias=False)
        self.ml = torch.nn.Sequential(*(list(self.ml.children())[:-1]))
