...
class Model_3D(nn.Module):
    def __init__(self, in_channels, out_channels, num_layer, num_answer, drop, input):
        ...
        for l in range(self.layers):
            decode_step = nn.GRUCell(init_dim, decoder_dim, bias=True)  # decoding LSTMCell
    ...
    def forward(self, x) :
        ...
        for t in range(decode_lengths):
            ...
            input_h = attention_weighted_encoding
            for l in range(self.layers):
                input_h = self.decode_steps[l](input_h, hs[l])  # (batch_size, decoder_dim)
                hs[l] = input_h
